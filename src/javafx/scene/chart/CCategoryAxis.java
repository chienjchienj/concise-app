 ...
 * @since JavaFX 2.0
 */
public final class CCategoryAxis extends Axis<String> {
    ...
    private DoubleProperty startMargin = new StyleableDoubleProperty(5) {
        ...
        @Override public CssMetaData<CCategoryAxis,Number> getCssMetaData() {
            return StyleableProperties.START_MARGIN;
        }
        
        @Override
        public Object getBean() {
            return CCategoryAxis.this;
        }
    ...
    private BooleanProperty gapStartAndEnd = new StyleableBooleanProperty(true) {
        ...
        @Override public CssMetaData<CCategoryAxis,Boolean> getCssMetaData() {
     ...
     */
    public CCategoryAxis(ObservableList<String> categories) {
...
//        if (categories.isEmpty()) new java.lang.Throwable().printStackTrace();
    ...
    private double calculateRequiredSize(boolean axisVertical, double tickLabelRotation) {
        ...
        for (String category: allDataCategories) {
            Dimension2D textSize = measureTickMarkSize(getTickMarkLabel(category), tickLabelRotation);
    ...
    @Override protected String getTickMarkLabel(String value) {
        // TODO use formatter
        return value.substring(0, Math.min(value.length(), 5)) + (value.length() > 5 ? "..." : "");
    }
    ...
    @Override protected Dimension2D measureTickMarkSize(String value, Object range) {
        ...
        return measureTickMarkSize(getTickMarkLabel(value),tickLabelRotation);
    }
...
//        for(String cat : data) {
    ...
    private static class StyleableProperties {
        private static final CssMetaData<CCategoryAxis,Number> START_MARGIN =
            new CssMetaData<CCategoryAxis,Number>("-fx-start-margin",
                SizeConverter.getInstance(), 5.0) {

            @Override
            public boolean isSettable(CCategoryAxis n) {
                return n.startMargin == null || !n.startMargin.isBound();
            }

            @Override
            public StyleableProperty<Number> getStyleableProperty(CCategoryAxis n) {
                return (StyleableProperty<Number>)n.startMarginProperty();
            }
        };
        
        private static final CssMetaData<CCategoryAxis,Number> END_MARGIN =
            new CssMetaData<CCategoryAxis,Number>("-fx-end-margin",
                SizeConverter.getInstance(), 5.0) {
        ...
        private static final CssMetaData<CCategoryAxis,Boolean> GAP_START_AND_END =
            new CssMetaData<CCategoryAxis,Boolean>("-fx-gap-start-and-end",
            ...
            @Override
            public StyleableProperty<Boolean> getStyleableProperty(CCategoryAxis n) {
